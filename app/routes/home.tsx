                        ...
                        <Card>
                            <CardContent className="pt-6">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div
                                        className="h-12 w-12 bg-brand/10 rounded-full flex items-center justify-center">
                        ...
                        <Card>
                            <CardContent className="pt-6">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div
                                        className="h-12 w-12 bg-[#FFB30D]/10 rounded-full flex items-center justify-center">
...
function ProjectCard({project, currentUser}: { project: Project; currentUser?: User }) {
    ...
    // Find current user's role by matching their userId with the project's users array
    const currentUserRole = currentUser
        ? project.users.find(u => u.id === currentUser._id)?.role || "unknown"
        : "unknown";

    // Format role for display
    const getRoleBadge = (role: string) => {
        const roleConfig = {
            admin: {label: "Admin", icon: Shield, color: "bg-[#FFB30D]/10 text-[#FFB30D]"},
            editor: {label: "Editor", icon: Shield, color: "bg-blue-100 text-blue-700"},
            viewer: {label: "Viewer", icon: Shield, color: "bg-gray-100 text-gray-700"},
        };
    ...
    const roleBadge = getRoleBadge(currentUserRole);
        ...
        <Card className="hover:shadow-lg transition-shadow border-brand/20">
            <CardHeader>
                <div className="flex items-start justify-between">
                    ...
                    <span
                        className={`inline-flex items-center gap-1 px-2 py-1 ${roleBadge.color} text-xs font-medium rounded`}>
            ...
            <CardContent className="space-y-4">
                ...
                {currentUser!._id !== project.ownerId
                    && <div className="text-xs text-gray-500">
                        <strong>Owned by</strong>: {project.ownerName} ({project.ownerEmail})
                    </div>}
...
function FeatureCard({icon, title, description}: { icon: React.ReactNode; title: string; description: string }) {
    return (
        <Card className="bg-white/10 backdrop-blur-sm border-white/20 hover:bg-white/15 transition-colors">
            <CardContent className="pt-6">
                <div
                    className="h-12 w-12 bg-[#FFB30D]/20 rounded-lg flex items-center justify-center mb-4 text-[#FFB30D]">
                    {icon}
                </div>
